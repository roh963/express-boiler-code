...
export const listFeedback = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json(new ApiResponse(200, result.data, undefined, result.meta));
});
...
export const getFeedback = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!feedback) {
    return res.status(404).json({ errors: [{ field: 'id', message: 'Feedback not found' }] });
  }
...
export const createFeedback = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(201).json(new ApiResponse(201, feedback, 'Feedback created'));
});
...
export const updateFeedback = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json(new ApiResponse(200, updated, 'Feedback updated'));
});
